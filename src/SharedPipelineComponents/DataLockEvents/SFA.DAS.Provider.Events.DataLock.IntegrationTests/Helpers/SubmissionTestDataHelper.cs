...
{
    public class SubmissionTestDataHelper : ITestDataHelper
    {
        public void Clean()
        {
        ...
        {
            CommonTestDataHelper.AddCommitment(id, ukprn, learnerRefNumber, aimSequenceNumber, uln, startDate, endDate, agreedCost, standardCode, programmeType, frameworkCode, pathwayCode, passedDataLock);
        }

        public void AddIlrDataForCommitment(long? commitmentId, string learnerRefNumber, int aimSequenceNumber = 1)
        {
            CommonTestDataHelper.AddIlrDataForCommitment(commitmentId, learnerRefNumber, aimSequenceNumber);
        }
        ...
        {
            return CommonTestDataHelper.GetAllEvents(true);
        }

        public DataLockEventError[] GetAllEventErrors(Guid eventId)
        {
            return CommonTestDataHelper.GetAllEventErrors(eventId, true);
        }

        public DataLockEventPeriod[] GetAllEventPeriods(Guid eventId)
        {
            return CommonTestDataHelper.GetAllEventPeriods(eventId, true);
        }

        public DataLockEventCommitmentVersion[] GetAllEventCommitmentVersions(Guid eventId)
        {
            return CommonTestDataHelper.GetAllEventCommitmentVersions(eventId, true);
        }
    }
}
