...
{
    public class PeriodEndTestFixtureDataHelper : ITestFixtureDataHelper
    {
        public void Clean()
        {
        ...
        {
            TestDataHelper.PeriodEndAddCommitment(id, ukprn, learnerRefNumber, aimSequenceNumber, uln, startDate, endDate, agreedCost, standardCode, programmeType, frameworkCode, pathwayCode, passedDataLock);
        }

        public void AddIlrDataForCommitment(long? commitmentId, string learnerRefNumber, int aimSequenceNumber = 1)
        {
            TestDataHelper.PeriodEndAddIlrDataForCommitment(commitmentId, learnerRefNumber, aimSequenceNumber);
        }
        ...
        {
            TestDataHelper.AddReferenceDataLockEvent(ukprn, status, false);
        }

        public DataLockEvent[] GetAllEvents()
        {
            return TestDataHelper.GetAllEvents(false);
        }

        public DataLockEventError[] GetAllEventErrors(Guid eventId)
        {
            return TestDataHelper.GetAllEventErrors(eventId, false);
        }

        public DataLockEventPeriod[] GetAllEventPeriods(Guid eventId)
        {
            return TestDataHelper.GetAllEventPeriods(eventId, false);
        }

        public DataLockEventCommitmentVersion[] GetAllEventCommitmentVersions(Guid eventId)
        {
            return TestDataHelper.GetAllEventCommitmentVersions(eventId, false);
        }
